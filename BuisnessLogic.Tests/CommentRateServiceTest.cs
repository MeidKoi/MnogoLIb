...
namespace BuisnessLogic.Tests
{
    public class CommentRateServiceTest
    {
        ...
        [Fact]
        public async void GetByIdAsync_NullCommentRate_ShullThrowArgumentExpression()
        {
            var ex = await Assert.ThrowsAnyAsync<ArgumentNullException>(() => service.GetById(-1, -1));

            Assert.IsType<ArgumentNullException>(ex);
            repMoq.Verify(x => x.FindByCondition(It.IsAny<Expression<Func<CommentRate, bool>>>()), Times.Once);
        }


        [Fact]
        public async void DeleteAsync_NullCommentRate_ShullThrowArgumentExpression()
        {
            var ex = await Assert.ThrowsAnyAsync<ArgumentNullException>(() => service.Delete(-1, -1));

            Assert.IsType<ArgumentNullException>(ex);
            repMoq.Verify(x => x.Delete(It.IsAny<CommentRate>()), Times.Never);
        }
