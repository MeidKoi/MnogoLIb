...
namespace BuisnessLogic.Tests
{
    public class CommentServiceTest
    {
        ...
        [Fact]
        public async void GetByIdAsync_NullComment_ShullThrowArgumentExpression()
        {
            var ex = await Assert.ThrowsAnyAsync<ArgumentNullException>(() => service.GetById(-1));

            Assert.IsType<ArgumentNullException>(ex);
            repMoq.Verify(x => x.FindByCondition(It.IsAny<Expression<Func<Comment, bool>>>()), Times.Once);
        }


        [Fact]
        public async void DeleteAsync_NullComment_ShullThrowArgumentExpression()
        {
            var ex = await Assert.ThrowsAnyAsync<ArgumentNullException>(() => service.Delete(-1));

            Assert.IsType<ArgumentNullException>(ex);
            repMoq.Verify(x => x.Delete(It.IsAny<Comment>()), Times.Never);
        }
